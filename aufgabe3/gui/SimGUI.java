...
public class SimGUI extends JFrame implements Runnable,ActionListener,ChangeListener, WindowListener {
	...
	private TreeMap<Integer, gui.Station> stations;
	private Vector<gui.MapEntry> mapEntries;
	private Vector<gui.MapEntry> mapEntriesReplay;
	...
	public SimGUI() throws IOException {
		stations = new TreeMap<Integer, gui.Station>();
		readStationsFromFile(this.getClass().getResource("stations.txt").getPath());
		mapEntries = new Vector<gui.MapEntry>();
		mapEntriesReplay = new Vector<gui.MapEntry>();
		lock = new Object();
		simState = seqStopped;
	...
	private void prepareGUI() throws IOException {
        ...
        tbar.setState(Toolbar.ToolbarState.STOPPED);
	...
	private void readStationsFromFile(String filename) throws FileNotFoundException {
        ...
        while (in.hasNext()) {
            ...
            int x = in.nextInt();
            int y = in.nextInt();
            stations.put(nr, new gui.Station(x, y));
        }
        in.close();
        for (int i = 1; i <= 199; i++) {
	...
	public void drive(int start, int stop, Color color) {
		if (!simAborted && userState == seqStarted) {
			gui.Station s1 = stations.get(start);
			gui.Station s2 = stations.get(stop);
			if (s1 == null || s2 == null) {
				return;
			}
			synchronized(lock) {
				mapEntries.add(new gui.Route(s1, s2, color));
			}
		}
	}
	...
	public void run() {
		...
		while (!simAborted && simState != seqStopped) {
			...
			switch (simState) {
				...
				case seqRunning:
					tbar.setState(Toolbar.ToolbarState.PLAY);
					// no entry to draw
				...
				case seqPaused:
					// pause drawing
					tbar.setState(Toolbar.ToolbarState.PAUSED);
					break;
				
				case seqFinished:
					// simulation finished, stop drawing
					tbar.setState(Toolbar.ToolbarState.FINISHED);
					break;
	...
	public void actionPerformed(ActionEvent e) {		
		if (Toolbar.CMD_PLAY.equals(e.getActionCommand())) {
			if (simState == seqFinished) {
				...
				mapEntries = (Vector<gui.MapEntry>) mapEntriesReplay.clone();
				mapEntriesReplay.clear();
				try {
					map.clearMap();
